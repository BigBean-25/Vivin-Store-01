...
export default function Categories() {
  ...
  const filteredCategories = useMemo(() => categories.filter((category) => `${category.name || ""} ${category.slug || ""} ${category.description || ""} ${category.status || ""}`.toLowerCase().includes(search.toLowerCase())), [categories, search]);
  ...
  return (
    <AdminLayout>
      <div className="category-page">
        ...
        <div className="stats-grid"><div className="stat-card"><h3>{categories.length}</h3><p>Total Categories</p></div><div className="stat-card"><h3>{categories.filter((c) => c.status === "active").length}</h3><p>Active Categories</p></div><div className="stat-card"><h3>{categories.filter((c) => c.status === "inactive").length}</h3><p>Inactive Categories</p></div></div>
