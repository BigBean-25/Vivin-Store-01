...
const getEntryTypeKey = (type) => {
  const value = String(type || "debit").toLowerCase().trim();
  return value === "credit" ? "credit" : "debit";
};

const getEntryTypeLabel = (type) => {
  const value = getEntryTypeKey(type);
...
export default function CustomerLedgers() {
  ...
  const fetchLedgers = async () => {
    try {
      ...
      const res = await API.get("/api/customer-ledgers");

      if (res.data.success) {
        setLedgers(res.data.ledgers || []);
      }
    } catch (err) {
      setError(err.response?.data?.message || "Failed to fetch customer ledgers");
    } finally {
      setLoading(false);
    }
  };
  ...
  const totals = useMemo(() => {
    const totalDebit = ledgers
      .filter((item) => getEntryTypeKey(item.entry_type) === "debit")
      .reduce((sum, item) => sum + Number(item.amount || 0), 0);

    const totalCredit = ledgers
      .filter((item) => getEntryTypeKey(item.entry_type) === "credit")
      .reduce((sum, item) => sum + Number(item.amount || 0), 0);

    const latestBalance =
      ledgers.length > 0 ? Number(ledgers[0]?.balance_after || 0) : 0;
    ...
    return [
      {
        ...
        value: formatNumber(ledgers.length),
        hint: "Total account records",
        icon: BookOpen,
        color: "#D9A900",
      },
      {
        ...
        hint: "Invoice / outstanding added",
        icon: TrendingDown,
        color: "#DC2626",
      },
      {
      ...
      },
      {
        ...
        value: formatCurrency(totals.latestBalance),
        hint: "Latest running balance",
        icon: Wallet,
        color: "#2563EB",
      },
    ];
  }, [ledgers.length, totals]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        entry_type: getEntryTypeKey(formData.entry_type),
        amount: Number(formData.amount || 0),
        reference_id: formData.reference_id || "",
      };

      const res = await API.post("/api/customer-ledgers", payload);
      ...
      setError(err.response?.data?.message || "Failed to create ledger entry");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <AdminLayout>
      ...
      <div className="ledger-page ledger-command-page">
        <section className="ledger-hero">
          ...
          <div className="hero-left">
            ...
            <div className="hero-copy">
              ...
              <h1>Customer Ledgers</h1>

              <p>
                Maintain customer account statements, debit entries, payment
                credits and running outstanding balance from one premium control
                panel.
              </p>
            </div>
          </div>
          ...
          <div className="hero-actions">
            <button
              className="ledger-secondary-btn"
              type="button"
        ...
        {showForm && (
          <section className="form-card">
            <div className="form-header">
              <div>
                <span className="section-label">Create Ledger Entry</span>

                <h2>Add Customer Ledger Entry</h2>

                <p>
                  Add debit for invoice/outstanding and credit for payment
                  received against customer account.
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    required
                    className={`entry-select entry-select-${getEntryTypeKey(
                      formData.entry_type
                    )}`}
                  >
                    <option value="debit">Debit / Invoice Amount</option>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="invoice / payment / manual"
                  />
                </div>
                ...
                <div className="form-group full">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    placeholder="Invoice amount added to customer ledger"
                  />
                </div>
              </div>
              ...
              <div className="form-note">
                <IndianRupee size={15} />
                Debit increases customer outstanding. Credit reduces customer
                outstanding.
              </div>
              ...
              <div className="form-actions">
                <button
                  type="button"
                  className="ledger-secondary-btn form-secondary"
                  onClick={handleCancelForm}
                >
                  Cancel
                </button>
        ...
        <section className="toolbar">
          ...
          <div className="toolbar-count">
            Showing <strong>{filteredLedgers.length}</strong> of{" "}
            <strong>{ledgers.length}</strong> ledger entries
          </div>
        </section>
        ...
        <section className="table-card">
          <div className="table-header">
            <div>
              <span className="section-label">Ledger Database</span>
              <h2>Customer Ledger Statement</h2>
              <p>Customer outstanding statement records from MySQL database</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="empty-box">
              ...
              <div>
                ...
                <p>Click Add Ledger Entry to create your first record.</p>
              </div>
            </div>
          ) : (
              ...
              <table>
                ...
                <tbody>
                  {filteredLedgers.map((item) => {
                    const entryType = getEntryTypeKey(item.entry_type);
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>
                          <span className={`type-badge type-${entryType}`}>
                            {entryType === "debit" ? (
...
const css = `
  ...
  .theme-dark .ledger-command-page {
    --page-text: #F8FAFC;
  ...
  .theme-dark .entry-select-debit {
    background: rgba(220, 38, 38, 0.10) !important;
    border-color: rgba(220, 38, 38, 0.28) !important;
    color: #FCA5A5 !important;
  }

  .theme-dark .entry-select-credit {
  ...
  @media (max-width: 760px) {
    ...
    .hero-actions .ledger-primary-btn,
    .hero-actions .ledger-secondary-btn {
      flex: 1;
    }
    ...
    .form-actions .ledger-primary-btn,
    .form-actions .ledger-secondary-btn {
      width: 100%;
    }
