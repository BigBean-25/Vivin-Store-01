...
export default function WarehouseRacks() {
  ...
  const fetchSummary = async () => {
    try {
      const res = await API.get("/api/warehouse-racks/summary");
  ...
  const fetchRacks = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to fetch warehouse racks");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredZones = useMemo(() => {
    if (!formData.warehouse_id) return zones;
    return zones.filter((z) => String(z.warehouse_id) === String(formData.warehouse_id));
  }, [zones, formData.warehouse_id]);

  const filterZonesByWarehouse = useMemo(() => {
    if (!warehouseFilter) return zones;
    return zones.filter((z) => String(z.warehouse_id) === String(warehouseFilter));
  ...
  const filteredRacks = useMemo(() => {
    return racks.filter((rack) => {
      ...
      const matchesWarehouse = !warehouseFilter || String(rack.warehouse_id) === String(warehouseFilter);
      const matchesZone = !zoneFilter || String(rack.zone_id) === String(zoneFilter);

      return matchesSearch && matchesStatus && matchesWarehouse && matchesZone;
    });
  }, [racks, search, statusFilter, warehouseFilter, zoneFilter]);

  const stats = useMemo(() => ({
    total: summary.total_racks || 0,
    active: summary.active_racks || 0,
    inactive: summary.inactive_racks || 0,
    zones: summary.zones_with_racks || 0,
  }), [summary]);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (name === "warehouse_id") updated.zone_id = "";
      return updated;
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.rack_name.trim()) { setError("Rack name is required"); return; }
    ...
    try {
      ...
      if (editingId) {
        await API.put(`/api/warehouse-racks/${editingId}`, formData);
        showSuccess("Warehouse rack updated successfully");
      } else {
        await API.post("/api/warehouse-racks", formData);
        showSuccess("Warehouse rack created successfully");
      }

      handleCancelForm();
      refreshAll();
    } catch (err) {
      setError(err.response?.data?.message || "Failed to save warehouse rack");
    } finally {
      setSaving(false);
    }
  };

  const handleToggleStatus = async (rack) => {
    const newStatus = rack.status === "active" ? "inactive" : "active";
    if (!window.confirm(`${newStatus === "inactive" ? "Deactivate" : "Activate"} rack "${rack.rack_name}"?`)) return;
    try {
      setError("");
      await API.patch(`/api/warehouse-racks/${rack.id}/status`, { status: newStatus });
      showSuccess(`Rack ${newStatus} successfully`);
      refreshAll();
    } catch (err) {
      setError(err.response?.data?.message || "Failed to update rack status");
    }
  };

  const handleDelete = async (rack) => {
    if (!window.confirm(`Delete rack "${rack.rack_name}"? This cannot be undone.`)) return;
    try {
      setError("");
      await API.delete(`/api/warehouse-racks/${rack.id}`);
      showSuccess("Warehouse rack deleted successfully");
      refreshAll();
    } catch (err) {
      setError(err.response?.data?.message || "Failed to delete warehouse rack");
    }
  };
    ...
    <AdminLayout>
      <div className="wr-page">
        ...
        <div className="wr-hero">
          <div className="hero-left">
            ...
            <div>
              ...
              <p>
                Manage racks within warehouse zones. Racks organise physical
                storage locations and connect with bins for precise inventory
                placement.
              </p>
            </div>
          </div>
        ...
        <div className="stats-grid">
          <StatCard title="Total Racks" value={stats.total} />
          <StatCard title="Active Racks" value={stats.active} />
          <StatCard title="Inactive Racks" value={stats.inactive} />
          <StatCard title="Zones with Racks" value={stats.zones} />
        </div>
        ...
        {showForm && (
          <div className="form-card">
            <div className="form-header">
              <div>
                <h2>{editingId ? "Edit Rack" : "Add New Rack"}</h2>
                <p>Define rack details under a warehouse zone.</p>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-grid">
                ...
                <div className="form-group full">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows="3"
                    placeholder="Rack description or storage notes..."
                  />
                </div>
              </div>
              ...
              <div className="form-actions">
                ...
                <button type="submit" className="save-btn" disabled={saving}>
                  ...
                  {saving ? "Saving..." : editingId ? "Update Rack" : "Save Rack"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="toolbar">
          <div className="search-wrap">
            ...
            <input
              ...
              placeholder="Search rack code, name, zone, warehouse..."
            />
          </div>
          ...
          <div className="api-chip">
            API Connected · <strong>{filteredRacks.length}</strong> records
          </div>
        </div>
        ...
        <div className="table-card">
          <div className="table-header">
            <h2>Rack List</h2>
            <p>Warehouse rack master records from MySQL database</p>
          </div>
          ...
          {loading ? (
            <div className="empty-box">
              ...
              <h3>Loading racks...</h3>
              <p>Please wait while rack records are loading.</p>
            </div>
          ) : filteredRacks.length === 0 ? (
            <div className="empty-box">
              <Package size={34} />
              <h3>No racks found</h3>
              <p>Click Add Rack to create your first warehouse rack.</p>
            </div>
          ) : (
              ...
              <table>
                ...
                <tbody>
                  {filteredRacks.map((rack) => (
                    <tr key={rack.id}>
                      <td>
                        <div className="rack-name">{rack.rack_name}</div>
                        <div className="small-text">Code: {rack.rack_code || "-"}</div>
                        {rack.description && (
                          <div className="desc-text">{rack.description}</div>
                        )}
                      </td>
                      <td>
                        ...
                        <div className="small-text">{rack.zone_code || ""}</div>
                      </td>
                      <td>
                        <div className="info-line">
                          <Warehouse size={13} />
                        ...
                        <div className="small-text">{rack.warehouse_code || ""}</div>
                      </td>
                      <td>
                        <div className="small-text">
                          {rack.capacity > 0 ? Number(rack.capacity).toLocaleString() : "-"}
                        </div>
                      </td>
                      <td>
                        <div className="small-text">{formatDate(rack.created_at)}</div>
                      </td>
                      <td>
                      ...
                      <td>
                        <div className="action-buttons">
                          ...
                          <button
                            type="button"
                            className={`toggle-btn ${rack.status}`}
                            onClick={() => handleToggleStatus(rack)}
                            title={rack.status === "active" ? "Deactivate" : "Activate"}
                          >
                            {rack.status === "active" ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                          </button>
                          <button
                            type="button"
...
const css = `
  ...
  .wr-hero::after {
    ...
    border-radius: 50%;
    border: 42px solid rgba(16,185,129,0.08);
    right: -70px; top: -90px;
  }
  ...
  .wr-hero h1 { font-size: 26px; font-weight: 700; margin: 0 0 5px; }
  .wr-hero p { font-size: 13.5px; color: rgba(255,255,255,0.6); margin: 0; max-width: 520px; }
  ...
  .secondary-btn {
    ...
    font-size: 14px; font-weight: 500; cursor: pointer; transition: background 0.18s;
  }
  ...
  .stat-mark { width: 8px; height: 40px; background: #10b981; border-radius: 4px; opacity: 0.6; }
  ...
  .close-btn {
    ...
    padding: 7px; cursor: pointer; color: #374151; display: flex; align-items: center;
  }
  ...
  .form-group textarea {
    ...
    font-size: 14px; color: #111; outline: none; transition: border-color 0.15s;
  ...
  .cancel-btn {
    padding: 10px 18px; border: 1px solid #d1d5db; border-radius: 9px;
    background: #fff; font-size: 14px; font-weight: 500; cursor: pointer; color: #374151;
  }
  .save-btn {
    display: flex; align-items: center; gap: 7px;
  ...
  .search-wrap {
    ...
    padding: 9px 13px; color: #6b7280;
  }
  .search-wrap input {
    border: none; outline: none; font-size: 14px; flex: 1; color: #111; background: transparent;
  }
  .filter-select {
    border: 1px solid #d1d5db; border-radius: 9px; padding: 9px 13px;
    font-size: 14px; color: #374151; background: #fff; cursor: pointer; outline: none;
  }
  .api-chip {
    font-size: 12.5px; color: #6b7280; background: #f9fafb;
    border: 1px solid #e5e7eb; border-radius: 8px; padding: 7px 13px;
  }
  ...
  .empty-box {
    ...
    padding: 56px 24px; color: #9ca3af; gap: 10px; text-align: center;
  }
  ...
  thead th {
    background: #f9fafb; padding: 11px 16px; font-size: 12px; font-weight: 600;
  ...
  .rack-name { font-weight: 600; color: #111; font-size: 14px; }
  ...
  .edit-btn, .delete-btn, .toggle-btn {
    border: none; border-radius: 7px; padding: 6px 8px;
